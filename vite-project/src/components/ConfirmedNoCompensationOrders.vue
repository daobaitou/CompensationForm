...
const tableData = ref([])
const expectedStatus = OrderStatus.CONFIRMED_NO_COMPENSATION
...
onMounted(async () => {
  try {
    console.log('开始获取确认无需赔付订单数据，状态:', expectedStatus);
    tableData.value = await fetchDataByStatus(expectedStatus);
    console.log('获取到的确认无需赔付订单数据:', tableData.value);
  } catch (error) {
    console.error('获取确认无需赔付订单数据失败:', error);
  }
});
</script>
